...
describe('AgentExecutorService', () => {
  ...
  it('executes carousel playbook with all inputs', async () => {
    const playbook = PLAYBOOK_DEFINITIONS.find(
      (p) => p.id === 'carousel_template_failed',
    )!;
  ...
  it('handles non-existent user gracefully', async () => {
    ...
    const step = result.trace.stepsExecuted.find(
      (s) => s.toolResult?.toolName === 'users.findByEmail',
    );
  ...
  it('handles disabled feature flag', async () => {
    ...
    expect(
      result.trace.stepsExecuted.some(
        (s) => s.toolResult?.toolName === 'gitlab.checkFeatureFlag',
      ),
    ).toBe(true);
