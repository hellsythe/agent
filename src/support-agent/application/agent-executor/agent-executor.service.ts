...
import type { ToolExecutionResult, ToolRegistryPort } from '../../domain/tool-definition.entity';
...
export class AgentExecutorService {
  ...
  async execute(input: AgentExecutorInput): Promise<AgentExecutorOutput> {
    ...
    const missing = this.detectMissingInputs(input.playbook, input.initialInputs);
    if (missing.length > 0) {
      this.logger.info('Agent executor missing inputs', { playbookId: input.playbook.id, missing });
      return {
    ...
    for (const step of input.playbook.steps) {
      const stepResult = await this.executeStep(step, trace.finalInputs, input.playbook.allowedTools);
      trace.stepsExecuted.push(stepResult);
      if (stepResult.toolResult?.data) {
        if (step.tool === 'templates.findByName' && stepResult.toolResult.data.type && !stepResult.toolResult.data.templateType) {
          stepResult.toolResult.data.templateType = stepResult.toolResult.data.type;
        }
      ...
      if (stepResult.toolResult?.success === false) {
        this.logger.warn('Tool execution failed', { stepId: step.id, tool: step.tool, error: stepResult.toolResult.error });
    ...
    this.logger.info('Agent executor completed', { playbookId: input.playbook.id, diagnosis: trace.diagnosis });
  ...
  private detectMissingInputs(playbook: Playbook, inputs: Record<string, unknown>): string[] {
  ...
  private async executeStep(
    ...
    if (!allowedTools.includes(step.tool)) {
      return {
        ...
        toolResult: { toolName: step.tool, success: false, error: 'Tool not allowed by playbook', durationMs: 0 },
      };
    }
  ...
  private mapInputsForTool(toolName: string, inputs: Record<string, unknown>): Record<string, unknown> {
    const mapping: Record<string, Record<string, string>> = {
      'clients.findByName': { name: 'clientName' },
      'clients.resolveClient': { clientName: 'clientName', clientId: 'clientId' },
      'clients.getDeliveryRuntime': { clientId: 'clientId', requestedChannel: 'channel' },
  ...
  private buildResolution(playbook: Playbook, inputs: Record<string, unknown>): string {
