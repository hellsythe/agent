...
describe('ToolRegistryService', () => {
  ...
  it('returns all available tools', () => {
    ...
    expect(tools.some((t) => t.name === 'clients.getDeliveryRuntime')).toBe(true);
    expect(tools.some((t) => t.name === 'bypass.checkClient')).toBe(true);
  });

  it('executes clients.findByName successfully', async () => {
    const result = await registry.executeTool('clients.findByName', { name: 'chedraui' });
    expect(result.success).toBe(true);
    expect(result.data).toHaveProperty('clientCandidates');
  });

  it('executes clients.resolveClient with numeric clientId', async () => {
    const result = await registry.executeTool('clients.resolveClient', { clientId: '49980' });
    expect(result.success).toBe(true);
    expect(result.data).toMatchObject({ clientId: '49980', clientName: 'Aurrera' });
  });
  ...
  it('executes bypass.checkClient and returns false for chedraui', async () => {
    const result = await registry.executeTool('bypass.checkClient', { clientId: 'client-chedraui-mx' });
    expect(result.success).toBe(true);
    expect(result.data).toEqual({ bypassEnabled: false, clientId: 'client-chedraui-mx' });
  });

  it('executes gitlab.checkFeatureFlag', async () => {
    const result = await registry.executeTool('gitlab.checkFeatureFlag', { featureName: 'sender_v2' });
    expect(result.success).toBe(true);
    expect(result.data).toEqual({ enabled: true, featureName: 'sender_v2' });
  });

  it('returns error when feature flag does not exist', async () => {
    const result = await registry.executeTool('gitlab.checkFeatureFlag', { featureName: 'unknown_flag' });
    expect(result.success).toBe(true);
    expect(result.data).toEqual({ enabled: false, featureName: 'unknown_flag' });
  });

  it('executes users.findByEmail successfully', async () => {
    const result = await registry.executeTool('users.findByEmail', { email: 'test@example.com' });
    expect(result.success).toBe(true);
    expect(result.data).toHaveProperty('userId');
  });

  it('returns error when user does not exist', async () => {
    const result = await registry.executeTool('users.findByEmail', { email: 'nonexistent@example.com' });
    expect(result.success).toBe(false);
